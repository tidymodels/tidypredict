...
- `tidypredict_fit()` now reproduces `predict()` for an `aorsf::orsf()` model on the data it was trained on, where over half of the rows previously got a different prediction. aorsf splits on observed values of a linear combination of the predictors, so every training row lands exactly on one of its split thresholds, and aorsf computes that combination with a fused multiply-add, which no R or SQL expression can reproduce bit for bit; the last-bit difference sent the row down the other branch. The split thresholds are now calibrated against the training data so they fall between the two branches, which moves them by a bit or two at most and leaves predictions on new data unchanged. (#351)
