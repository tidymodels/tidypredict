...
- Categorical variables are supported, but only with *treatment* contrasts (`contr.treatment`).
...
The parser reads several parts of the `lm` object to tabulate all of the needed variables.  One entry per coefficient is added to the final table, those entries will have the results of `qr.solve()` already evaluated and placed in the correct column, they will have a `qr_` prefix.  There will be one `qr_` column per coefficient.
...
`tidypredict` also supports quantile regression models fitted with `quantreg::rq()`, as well as `linear_reg()` models fitted via `parsnip` with the `"quantreg"` engine.
