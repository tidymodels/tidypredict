...
XGBoost and Cubist store everything as 32-bit floats, making them most susceptible to this issue. LightGBM and CatBoost store leaf values as 64-bit doubles, so the values a prediction is built from carry full precision. Their split thresholds are still 32-bit floats, though, so the boundary problem described here applies to them as well: only the size of the discrepancy is reduced, not the chance of taking the wrong branch.
...
2. Use native predictions when possible: When you are not pushing predictions to a database, and perfect agreement is critical, consider using the native model's predict function rather than SQL translation.
