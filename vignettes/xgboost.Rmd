...
- Confirm that `tidypredict` results match the model's `predict()` results. The `xg_df` argument expects the `xgb.DMatrix` data set.
    ```{r}
tidypredict_test(model, mtcars, xg_df = xgb_bin_data)
    ```

Please be aware that XGBoost converts data into 32-bit floats internally. This could lead to prediction discrepancies at exact split boundaries. Always verify that predictions match using `tidypredict_test()`. See the [float precision](float-precision.html) article for more details.
