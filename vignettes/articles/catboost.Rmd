...
- Confirm that `tidypredict` results match the model's `predict()` results. The `xg_df` argument expects the matrix data set.
...
`tidypredict` always returns predictions on the response scale, so the link implied by the objective is inverted for you. This does not always agree with `catboost.predict()`, whose default `prediction_type = "RawFormulaVal"` returns the untransformed score. The equivalent `catboost.predict()` call is listed for each group below.
...
When using parsnip and bonsai, categorical features are handled automatically:

```{r, eval = eval_code && rlang::is_installed(c("parsnip", "bonsai"))}
library(parsnip)
library(bonsai)
