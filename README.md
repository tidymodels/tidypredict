...
database. Since it uses `dplyr`’s database interface, it works with
several database back-ends, such as MS SQL:
...
`tidypredict` has only a few functions and the number is not expected to grow much. The main focus as of now is to add more models
to support.
...
creates an R formula. The formula generated can then be used inside `dplyr`. The
...
`tidypredict` writes and reads a spec based on a model. Instead of writing the R formula directly, splitting the spec from the
