...
#> case_when(case_when(!is.na(cyl) ~ cyl < 5, !is.na(disp) ~ disp < 
#>     142.9, !is.na(hp) ~ hp < 101, !is.na(wt) ~ wt < 2.5425, !is.na(qsec) ~ 
#>     !qsec < 18.41, !is.na(vs) ~ !vs < 0.5, .default = FALSE) ~ 
#>     26.6636363636364, .default = case_when(case_when(!is.na(hp) ~ 
#>     hp < 192.5, !is.na(disp) ~ disp < 334, !is.na(wt) ~ wt < 
#>     4.66, !is.na(qsec) ~ !qsec < 15.455, !is.na(carb) ~ carb < 
#>     3.5, !is.na(gear) ~ gear < 4.5, .default = TRUE) ~ 18.2642857142857, 
...
#> case_when(case_when(!is.na(Petal.Length) ~ Petal.Length < 2.45, 
#>     !is.na(Petal.Width) ~ Petal.Width < 0.8, !is.na(Sepal.Length) ~ 
#>         Sepal.Length < 5.45, !is.na(Sepal.Width) ~ !Sepal.Width < 
#>         3.35, .default = FALSE) ~ "setosa", .default = case_when(case_when(!is.na(Petal.Width) ~ 
#>     Petal.Width < 1.75, !is.na(Petal.Length) ~ Petal.Length < 
#>     4.75, !is.na(Sepal.Length) ~ Sepal.Length < 6.15, !is.na(Sepal.Width) ~ 
#>     Sepal.Width < 2.95, .default = TRUE) ~ "versicolor", .default = "virginica"))
```
...
#> case_when(case_when(!is.na(cyl) ~ cyl %in% c("6", "8"), !is.na(hp) ~ 
#>     !hp < 101, !is.na(wt) ~ !wt < 2.5425, .default = TRUE) ~ 
#>     case_when(case_when(!is.na(hp) ~ hp < 192.5, !is.na(wt) ~ 
#>         wt < 4.66, .default = TRUE) ~ 18.2642857142857, .default = 13.4142857142857), 
