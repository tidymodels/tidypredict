# returns the right output
    ...
    Code
      rlang::expr_text(tf)
    Output
      [1] "ifelse(is.na(cyl) | is.na(disp) | is.na(hp) | is.na(drat) | is.na(wt) | \n    is.na(qsec) | is.na(vs) | is.na(am) | is.na(gear) | is.na(carb), \n    NA_real_, (case_when(wt <= 2.3325 ~ case_when(disp <= 78.85 ~ \n        33.525, .default = 27.425), .default = case_when(disp <= \n        281 ~ case_when(cyl <= 5 ~ case_when(drat <= 3.695 ~ \n        24.4, .default = 22.3666666666667), .default = case_when(wt <= \n        3.3275 ~ case_when(disp <= 152.5 ~ 19.7, .default = 21.1), \n        .default = case_when(disp <= 196.3 ~ 18.92, .default = 18.1))), \n        .default = 14.1)) + case_when(disp <= 266.9 ~ case_when(hp <= \n        80.5 ~ 28.5333333333333, .default = case_when(hp <= 118 ~ \n        case_when(wt <= 1.989 ~ 30.4, .default = case_when(hp <= \n            96 ~ 22.8, .default = case_when(qsec <= 19.725 ~ \n            21.4, .default = 21.5))), .default = case_when(qsec <= \n        18.6 ~ 19.6, .default = 17.8))), .default = case_when(drat <= \n        3.035 ~ 10.4, .default = case_when(carb <= 2.5 ~ 18.7, \n        .default = case_when(drat <= 3.635 ~ case_when(wt <= \n            4.5625 ~ 15.04, .default = 14.7), .default = 13.3)))) + \n        case_when(drat <= 3.75 ~ case_when(wt <= 3.4725 ~ 20.8333333333333, \n            .default = case_when(qsec <= 16.23 ~ 13.3, .default = case_when(disp <= \n                456 ~ case_when(qsec <= 17.225 ~ 19.2, .default = case_when(wt <= \n                4.7075 ~ 16.34, .default = 14.7)), .default = 10.4))), \n            .default = case_when(disp <= 130.55 ~ case_when(disp <= \n                78.85 ~ 32.2333333333333, .default = 27.75), \n                .default = case_when(cyl <= 5 ~ 22.8, .default = case_when(qsec <= \n                  17.66 ~ 21, .default = 19.2)))))/3)"
